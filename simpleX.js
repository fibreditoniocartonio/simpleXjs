...
/*Questi erano i bottoni per level++ e level--
      ...
      function playerPhysics(p1, lvl) {
        ...
        for(var i = 0; i < lvl.length; i++) {
          if(collisionBetween(p1, lvl[i])) {
            p1.y += -p1.yv;
            //dash
            if(keys[dashkey] && player.canMove) {
              p1.speed=p1.defaultspeed*2.5;
            }else{
              p1.speed=player.defaultspeed;
            }
            //jump
            if(keys[jumpkey] && player.canMove) {
              if(!p1.giasaltato) {
                p1.yv = -p1.jumpheight;
                p1.giasaltato = true;
              } else {
        ...
        //x movement
        if(keys[destrakey] && player.canMove) {
          p1.xv -= p1.speed;
        ...
        //shooting
        if(keys[sparokey] && player.canMove) {
          if(!player.giasparato){
            ...
            player.carica++;
            if (player.carica > 80){ //disegna i pallini del colore della carica intorno al player
